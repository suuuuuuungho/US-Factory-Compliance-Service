"""SUU-156: 서버가 켜질 때 청크(임베딩·본문) + BM25 검색 함수를 한 번만 메모리에 올린다.
...
def fetch_chunks(client, release_id: str) -> list[dict]:
    ...
    columns = "chunk_key,node_key,embedding,context_text,chunk_text"
    while True:
...
def load_index(client, release_id: str) -> Index:
    if release_id not in _CACHE:
        ...
        _CACHE[release_id] = Index(release_id, chunks, build_keyword_search(chunks))
