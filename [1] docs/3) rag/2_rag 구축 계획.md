...
2) eCFR
   ...
   - 예외 규칙: Subpart 하나가 컨텍스트 생성 모델의 한도(Haiku 4.5는 20만 토큰)를 넘으면 그 Subpart는 `subject_group`(중간 제목) 단위로 문서를 나누고, Subpart 제목·목차를 각 문서 앞에 붙인다. 어느 Subpart가 해당하는지는 `실측 후 기입`.
...
1) 모델과 호출 방법
   - 모델: Claude Haiku 4.5 (`claude-haiku-4-5`). 입력 $1, 출력 $5 (백만 토큰당). Anthropic 실험도 Haiku급을 썼다.
   - 문서 전체(Subpart)를 시스템 프롬프트에 넣고 `cache_control`로 캐싱한다. 청크마다 한 번씩 호출하되 문서 부분은 캐시에서 읽는다.
   - 최초 전체 색인은 Message Batches API로 보낸다. 비용이 절반이다. 갱신 시 바뀐 조문 몇 개는 일반 호출로 처리한다.
   - 캐시는 모델별 최소 길이(512~4,096토큰)가 있다. 아주 짧은 Subpart는 캐시가 안 걸릴 수 있는데, 짧으면 비용도 작으므로 그대로 둔다.
...
5) 비용
   - Anthropic 기준 약 $1.02/문서 100만 토큰. 우리 문서(Subpart)는 8천 토큰보다 훨씬 크므로 캐시 읽기 비율이 더 높다. Part 63 전체 비용은 `실측 후 기입`.
