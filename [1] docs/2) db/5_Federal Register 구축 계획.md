...
5) 1-5. 수집 결과
   ...
   - 전체 본문·PDF·보완 후보 목록의 운영 수집은 미실행이다.
   - 구현 후 결과에는 검색 조건·종료일·페이지 수·고유 문서 수·형식별 확보 수·미확보 목록을 적는다.
...
3) 2-3. 파서시 마주한 문제와 해결
   ...
   - 운영 파서는 미구현이다. 실제 추출 정확도를 측정하지 않았다.
...
5) 2-5. 파서 결과
   - 예정 산출물: `documents.jsonl`, `blocks.jsonl`, `cfr_references.jsonl`, `date_events.jsonl`, `relations.jsonl`, `quality_report.json`.
...
3) 3-3. DB 적재 시 마주한 문제와 해결
   - 실제 적재 미실행. 예상 충돌은 발행일+번호와 원본 URL 대조로 해결한다.
   - 상세 API가 다른 발행일을 반환하면 연결을 중단하고 해당 발행일의 공식 PDF·MODS를 사용해 검토한다.
   - 조문을 찾지 못한 문서는 삭제하지 않고 미해결 참조로 보관한다.

4) 3-4. DB 적재 실행 결과
   - 미실행. 기대 목록 수와 적재+보류 수 일치, 동일 문서 재실행 중복 0건, 정정문 보존, 이전 release 복구를 확인해야 한다.
