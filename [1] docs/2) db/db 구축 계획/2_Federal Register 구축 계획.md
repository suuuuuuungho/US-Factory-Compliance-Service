...
[전수조사 결과](<../db overview/2_db 전수조사 결과.md>)에 따라 40 CFR Part 63의 변경 근거를 모은다. `★`는 검색·연결용으로 채택한 원본 필드다. 원본 JSON과 문서는 별도로 보존한다. 운영 수집·메타데이터 적재는 2026-09-21에 실행했다(SUU-210). 자동 갱신(Step.4)은 아직 없다.
