...
이 문서는 [전수조사 결과](<../db overview/2_db 전수조사 결과.md>)를 바탕으로 만든 실행 계획이다. 규정 범위는 40 CFR Part 63 전체다. `★`는 검색·연결용으로 채택한 원본 필드다. 나머지 원본도 파일 그대로 보존한다. 아래 DB 구조는 Supabase(PostgreSQL)에 만들었다(SUU-61·64). 기준일 `2026-09-11` 원문을 수집·파싱·적재해 공개했다(SUU-35~44, 65~69). 자동 갱신(Step.4)은 아직 없다.
