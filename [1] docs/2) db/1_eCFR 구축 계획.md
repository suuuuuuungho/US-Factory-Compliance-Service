...
이 문서는 [데이터 명세](<../db overview/Data Specification_v1.md>)를 바탕으로 만든 실행 계획이다. 규정 범위는 40 CFR Part 63 전체다. `★`는 검색·연결용으로 채택한 원본 필드다. 나머지 원본도 파일 그대로 보존한다. 아래 DB 구조는 Supabase(PostgreSQL)에 만들었다(SUU-61·64). 기준일 `2026-09-11` 원문을 수집·파싱·적재해 공개했다(SUU-35~44, 65~69). 자동 갱신(Step.4)은 아직 없다.
...
2) 1-2. 수집 방법
   - 작업 위치: `[2] db/1) eCFR/`. 원본은 `raw/{기준일}/{sha256}/`, 파서 결과는 `parsed/{기준일}/{parser_version}/`에 저장한다(SUU-37·44). 실제 폴더: `raw/2026-09-11/`, `parsed/2026-09-11/v1/`.
...
5) 1-5. 수집 결과
   ...
   - 운영 수집 실행(SUU-38, 2026-09-15): 기준일 `2026-09-11`, `import_in_progress=false`, 상태 `succeeded`. 목차 JSON 9,395,482바이트와 Part 63 XML을 `raw/2026-09-11/`에 해시 폴더로 저장하고 `manifest.json`·`run.json`에 출처·상태·해시·시각을 남겼다.
   - 목차 개수: subpart 158, section 2,473, appendix 647, subject group 492.
   - 이미지·수식 첨부 파일 다운로드는 하지 않았다.
...
3) 2-3. 파서시 마주한 문제와 해결
   ...
   - 파서 구현: 노드(SUU-41), 블록(SUU-42), 문단 경로 `label_path`(SUU-43), jsonl 저장·목차 대조(SUU-44), 표 셀 경계 보존(SUU-83).
...
4) 2-4. 파서 합격 기준과 파서 실행 결과
   ...
   - 실행 결과(`parsed/2026-09-11/v1/quality_report.json`): 노드 종류별 개수가 목차와 100% 일치. 블록 71,700개. `label_status`는 ok 55,330 · inherited 16,330 · uncertain 40.
   - 같은 폴더의 최신 보고서(2026-09-17 재실행)는 `lost_text=2`로 `failed`다. 사라진 글 2건은 아직 원인을 정리하지 않았다.
   - 병합 표·수식·중첩 번호별 원문 대조 표본 검증과 두 번 실행 해시 비교는 하지 않았다.

5) 2-5. 파서 결과
   - 실제 산출물: `nodes.jsonl`, `blocks.jsonl`, `quality_report.json`(SUU-44).
   - `references.jsonl`(조문 인용)·`assets.jsonl`(이미지·수식 파일)은 만들지 않았다.
   - 블록 종류: paragraph 65,188 · citation 1,910 · extract 1,108 · image 1,026 · heading 996 · table 784 · formula 481 · note 193 · editorial_note 6 · footnote 6 · other 2.
...
3) 3-3. DB 적재 시 마주한 문제와 해결
   - Windows 긴 경로(260자 초과)에서 원본 저장이 실패했다 → 경로 처리를 고쳤다(SUU-62).
   - 예상 중단: 일부 테이블만 들어감 → release를 `staging/failed`로 유지하고 직전 정상 release를 계속 제공한다.
   - 예상 중복: 재실행 → 원본 해시와 고유키로 같은 자료를 두 번 만들지 않는다.

4) 3-4. DB 적재 실행 결과
   - `python ecfr_ingest.py`(SUU-69)로 등록(SUU-65) → 적재(SUU-66) → 공개(SUU-67)를 한 번에 실행했다(SUU-68).
   - release 1개: 기준일 `2026-09-11`, 파서 `v1`, `published`(2026-09-16), `common_dataset_current`가 가리킨다.
   - `ecfr_node` 3,771행(part 1 · subpart 158 · section 2,473 · appendix 647 · subject group 492), `ecfr_block` 71,700행.
   - `ecfr_reference`·`ecfr_asset`·`ecfr_history`·`ecfr_correction` 테이블은 만들었지만 0행이다.
   - 실패 주입 후 이전 release 유지 시험은 하지 않았다(release가 하나뿐).
...
1) 4-1. DB 적재 방법
   ...
   - 실행 환경은 하나의 스케줄러로 통일한다. 저장소에서 운영한다면 `.github/workflows/db-refresh.yml`을 추가하고 수동 재실행 입력도 제공한다. 이 파일은 아직 없다.
...
4) 4-4. DB 적재 실행 결과
   - 자동 적재 미실행. ‘최초 실행 → 변경 없음 → 실제 변경 → 다운로드 실패 → 파서 실패 → 중간 중단 후 재실행 → 되돌리기’를 검증한 뒤 자동화 완료로 기록한다.
