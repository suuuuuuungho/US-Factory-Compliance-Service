"""SUU-120: 평가셋 v2 `[6] rag/eval/rag_eval_case_v2.jsonl` (v1 32건 + 새 68건 이상 = 100건 이상) 검사.

규칙은 v1(`test_rag_eval_case.py`)과 같다. 조문 존재 여부는 같은 fixture와 대조한다.
"""
import json
import re
...
MIN_CASES = 100
MAX_PER_SUBPART = 3  # v1에 3건인 Subpart가 있음. 새로 넣은 건은 v1 포함 2건까지만(SUU-120)
...
def test_has_hundred_cases_with_all_fields():
    cases = load_cases()
