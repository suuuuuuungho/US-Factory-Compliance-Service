...
- 답변 단계(2026-09-19~): 답 모양·채점표는 `[1] docs/3) rag/4_rag 답변 품질 개선 과정.md`. 순서는 [14] A-행.
...
- **v2 = `rag_eval_case_v2.jsonl` 102건**(SUU-120): v1 32건 + 새 70건. 새 건은 SUU-115 초안 중 근거 문장이 회신에 100% 그대로 있는 243건 → 주 Subpart당 v1 포함 2건까지 75건 → Claude가 회신 원문과 대조(70 채택, 5 제외, 15 손봄). 주 Subpart 68개. 세부: `[5] tickets/2)RAG/1_feat/SUU-120.md`. 앞으로의 측정은 v2로 하고 v1 숫자와 섞지 않는다.
...
|---|---|---|---|
| `baseline` | 컨텍스트 **없이** `chunk_text`만 임베딩 → 벡터 | 로컬 `bge-m3`로 평가 때만 임베딩(`run_eval.py --embedder bge --no-context`, 캐시 `eval/cache/`). Supabase에 저장 안 함 | **SUU-133** 완료 |
...
| 2026-09-18_hybrid_v2_llm_gpt-4o-mini (SUU-135) | hybrid + 규칙 + LLM(gpt-4o-mini, 상위 20·3,000자) | v2 102건 | 0.882 | 0.961 | 0.645 | 0.836 | — | $0.17 |
| 2026-09-18_hybrid_v2_llm_gpt-5-mini (SUU-135) | hybrid + 규칙 + LLM(gpt-5-mini, 상위 20·3,000자) | v2 102건 | 0.931 | 0.961 | **0.692** | 0.836 | — | $0.58 |
| ↑ 저장된 gpt-5-mini 답을 `search_sections(llm=…)`로 리플레이 (SUU-136 코드 반영) | hybrid + 규칙 + LLM | v2 102건 | 0.931 | 0.961 | **0.692** | 0.836 | — | $0 |
...
답변(A) 단계. 답 모양·채점표·동점 폭은 `4_rag 답변 품질 개선 과정.md`(SUU-145).
...
| A-5 ② | 조문 수 5→10 | 완료(SUU-152, $1.79). `--top-n 10` + 프롬프트 v2 → **0.961 / 0.739 / 0.996 / 1.775**, 실패 5, 심판 0점 0건. 심판 +0.09(동점 폭 밖), 실패 9→5. Recall은 기준 0.74에 0.001 모자람(검색 Recall@10 0.804에 묶임). 지어냄 1건(mosaic, Subpart A 63.8~63.10) → A-5 ③ |
| A-5 ③ | Subpart A 항상 포함 | 보류(SUU-153, $2.27). 점수는 올랐지만(Recall 0.739→0.785) 검색 결과와 상관없이 특정 조문을 고정으로 끼워 넣는 건 "정답을 미리 알고 맞추는" 셈이라 서비스 방식으로 맞는지 확신이 없다. 비용도 $1.79→$2.27, 입력 토큰 27k→44k/건. 기록만 남기고 기준 조합은 상위 10 |
| A-5 ④ | 합격선 + CI | 완료(SUU-155, $0). `answer_pass_line.py` — 기준 조합(v2, 상위 10, 항상 A 없음, gpt-5-mini) 최신 run이 subpart 0.93 / recall 0.70 / grounded 0.96 / judge 1.71 아래면 `test_answer_pass_line.py`가 빨강. 조문 수 20·Claude 비교는 후속(선택) |
| A-5 | 개선 실험 + 합격선 | ①~④ 완료. 남은 후속(선택): 조문 수 20, 인용 강제 프롬프트, Claude 비교 |

그 뒤: 대시보드(SUU-113), ADI 회신 `adi_` 테이블 적재.
