...
- 2026-09-17: gpt-4o-mini 초안으로 100건까지 늘리려던 SUU-115는 접었다(초안 452건은 `rag_eval_case_v2.draft.jsonl`에 참고용으로만 남김, 커밋 안 함). 평가셋을 어떻게 만들지는 [14] 순서대로 다시 정한다. 그때까지 v1 32건을 쓴다.
...
3. 평가셋을 어떻게 만들지(몇 건, 어떻게 뽑고, 사람/AI가 어디까지 쓰는지). 재료는 SUU-114로 받은 회신 PDF 489건.
...
| C-3 | 평가셋 | **미정** ([13]-3) |
| C-4 | 세 조합 재측정 | vector 완료([9]). reranker·hybrid는 다음 티켓, Kanon 크레딧 ≈$3 |
| C-5 | 합격선 + CI 검사 | C-4 뒤 |
