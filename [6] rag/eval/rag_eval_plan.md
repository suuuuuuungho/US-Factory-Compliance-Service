...
- 작성일: 2026-09-17
- 상태: v1 (SUU-81 spec 직전). 실측 숫자가 나오면 [9] 결과 기록에 채운다.
- 상위 문서: `[1] docs/3) rag/2_rag 구축 계획.md` [10] 평가와 합격 기준. 이 문서는 그 절을 실행 가능한 수준으로 구체화한 것이다. 둘이 다르면 이 문서를 따르고 상위 문서를 고친다.
- 관련 티켓: SUU-46(평가셋), SUU-76(PoC 검색), SUU-81(1차 측정), 이후 baseline/hybrid/rerank 비교 티켓, 대시보드 티켓.

---

## [0] 한 줄 요약

**"공장 설명 질문 32개를 넣었을 때, 정답 조문이 검색 결과 상위 몇 번째에 나오는가"**를 설정(baseline → contextual → hybrid → rerank)마다 같은 방법으로 재고, 숫자와 실패 원인을 남겨 품질이 어떻게 좋아졌는지 보여준다.
...
| Q3 | 정답이 얼마나 위에 있는가 | 리랭커·구조 확장 필요 여부 판단 | MRR, 첫 정답 순위 중앙값 |
...
| 이름 | 정의 | 용도 |
|---|---|---|
| **loose Hit@k** (주 지표) | 정답 조문 중 **하나라도** 상위 k 조문 안에 있음 | 답변 생성이 시작될 수 있는가 |
| **strict Hit@k** (참고) | 정답 조문 **전부**가 상위 k 안에 있음 | 근거를 빠짐없이 모았는가 |
| **Recall@k** (참고) | (상위 k 안에 있는 정답 조문 수) / (정답 조문 수) — 케이스별로 계산해 평균 | strict보다 부드럽게 "몇 개나 찾았나" |
...
- **MRR@K** = 평균(1 / first_gold_rank), 없으면 0.
...
| `hybrid` | contextual 벡터 + BM25(`tsv`) → RRF(k=60) | `tsv` 채우기, 키워드 검색 함수 | 후속 |
| `rerank` | hybrid 상위 150 → Kanon 2 Reranker → 20 | Isaacus rerank 호출 | 후속 |
...
- 단계가 loose Hit@20 또는 MRR을 올리지 못하면 뺀다. 판단은 [6]의 쌍대 비교로 한다.
...
## [13] 아직 정하지 않은 것
1. 합격선. 상위 문서의 가칭(top-20 실패율 5% 이하, Subpart 95% 이상)은 32건에서는 각각 "실패 1건 이하", "실패 1건 이하"를 뜻한다. contextual 실측 후 확정한다.
2. `baseline` 임베딩을 어디에 저장할지(별도 테이블 vs 컬럼 추가). 재임베딩 티켓에서 정한다.
3. 평가셋 확장 방식(수동 계속 vs ADI/Dashboard 적재 후 반자동).
