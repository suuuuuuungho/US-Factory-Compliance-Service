...
| A-1 | 답변 생성 = 판정 기준표 | 완료(SUU-138). 상위 5조문 전문 → gpt-5-mini JSON(후보 Subpart + 기준·근거 조문 + 확인할 것, 판정 없음). v2 102건: Subpart 적중 0.882, 인용 Recall 0.649(상위 5 안에 있던 정답은 0.88 인용), 인용 근거율 0.995. $1.0 |

그 뒤: 답변 3층 심판(기준 설명이 조문에 있나, 로컬 LLM, SUU-138 다음 티켓), 대시보드(SUU-113), ADI 회신 `adi_` 테이블 적재.
